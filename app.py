...
def main() -> None:
    ...
    # ── Debug / Data Source panel ──
    with st.expander("📡 数据源详情", expanded=False):
        m5_df = data["m5"]
        cols = st.columns(4)
        cols[0].metric("Ticker", data["symbol"])
        cols[1].metric("最新收盘", f"{m5_df['Close'].iloc[-1]:.2f}" if not m5_df.empty else "N/A")
        ts = m5_df.index[-1] if not m5_df.empty else None
        cols[2].metric("最新时间", str(ts) if ts is not None else "N/A")
        cols[3].metric("K线数量", f"{len(m5_df):,}" if not m5_df.empty else "0")
        cols2 = st.columns(2)
        cols2[0].metric("数据来源", "Yahoo Finance (yfinance) — 实盘")
        cols2[1].metric("缓存状态", "⚠️ Demo/Static 数据已被禁用，当前为实盘行情")
        st.caption(f"更新时间：{data['last_updated']} · DXY状态：{data['dxy_status']} · 引擎：smartmoneyconcepts / fallback")
